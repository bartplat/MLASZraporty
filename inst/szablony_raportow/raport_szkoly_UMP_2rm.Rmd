...
subtitle: |
  ...
  | – na tle uczniów innych `r get(params$obiektWskazniki)$formy_gramatyczne[[1]]$szkola_lm` z miast
  | Unii Metropolii Polskich im. Pawła Adamowicza (UMP)
output:
  pdf_document:
    dev: cairo_pdf
    fig_caption: no
...
if (szkola$ponowny_wybor[[1]]$n >= progLiczebnosci) {
  cat(
    ...
    szkola$formy_gramatyczne[[1]]$szkola_lm,
    " z miast UMP. Na poniższym wykresie odpowiedzi uczestników badania z Państwa szkoły zostały zestawione z ich odpowiedziami.\n",
    sep = ""
  )
}
```
...
if (szkola$ponowny_wybor[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    " z miast UMP, którym zadano pytanie: \"Gdyby teraz mógł/mogła Pan/i wybrać szkołę, to czy ponownie wybrał(a)by Pan/i tę samą szkołę, w której obecnie się Pan/i uczy?\". Dane z wykresu przedstawiono szczegółowo w poniższej Tabeli:*",
    sep = ""
  )
  ...
  tab = tab1 %>% 
    ...
    kable("latex", booktabs = TRUE,
          ...
          align = "lcc") %>%
    kable_styling(position = wyrownanieTabWykr) %>% 
    add_header_above(c(" ", "Państwa szkoła" = 1, "Pozostałe szkoły z miast UMP" = 1))
  return(tab)
}
```
...
if (szkola$przyg_do_zaw[[1]]$n >= progLiczebnosci) {
  ...
  cat(
    ...
    " uczniów innych ",
    wojSzk$formy_gramatyczne[[1]]$szkola_lm,
    " z miast UMP, którym zadano pytanie: \"Jak Pan/i uważa, na ile szkoła przygotowała Pana/ią do wykonywania zawodu, którego się Pan/i w niej uczy?\". Dane z wykresu przedstawiono szczegółowo w poniższej Tabeli:*",
    sep = ""
  )
...
if (szkola$przyg_do_zaw[[1]]$n >= progLiczebnosci) {
  ...
  if (nrow(tab1) %in% 1:3) {
    cat(
      "Poniższa Tabela zawiera informacje jaka część uczniów oceniła, że szkoła przygotowała ich dobrze lub doskonale do wykonywania zawodów, których się w niej uczyli. Informacje są prezentowane w odniesieniu do zawodów, w których kształciło się co najmniej 10 respondentów. Wyniki uzyskane wśród uczniów Państwa szkoły zostały zestawione z odpowiedziami uczniów kształcących się w tych samych zawodach w innych szkołach z miast UMP.\n\n",
      sep = ""
    )
    ...
    tyt = cat(
              "**Tabela ",
              tab_num(),
              ": Odsetek uczniów, zdaniem których szkoła przygotowała ich do wykonywania zawodu, którego się w niej uczyli**\n\n",
              sep = ""
            )
    ...
    tyt = cat(
              "**Tabela ",
              tab_num(),
              ": Odsetek uczniów, zdaniem których szkoła przygotowała ich dobrze lub doskonale do wykonywania zawodu, którego się w niej uczyli**\n\n",
              sep = ""
            )
...
if ((szkola$ocena_pnz[[1]]$n >= progLiczebnosci) & (szkola$S1 == 2 | szkola$S1 == 3)) {
  ...
  tab_f = tab %>% 
    kable("latex", booktabs = TRUE, escape = FALSE, col.names = NULL,
          ...
          caption = tyt) %>%
    kable_styling(position = wyrownanieTabWykr) %>% 
    add_header_above(c("Forma PNZ", "Liczba respondentów" = 1))
  return(tab_f)
...
if (szkola$S1 == 1) {
  if (szkola$nauka_zawod[[1]]$n >= progLiczebnosci) {
    ...
    cat(
      ...
      " uczniów innych ",
      wojSzk$formy_gramatyczne[[1]]$szkola_lm,
      " z miast UMP, którym zadano pytanie: \"Jak Pan/i ocenia, czy nauka u tego pracodawcy przygotowała Pan/i do wykonywania pracy w zawodzie, którego się Pan/i uczył/a?\". Dane z wykresu przedstawiono szczegółowo w poniższej Tabeli:*",
      sep = ""
    )
...
if (szkola$S1 == 2 | szkola$S1 == 3) {
  if (szkola$plany_6m_stud[[1]]$n >= progLiczebnosci) {
    cat(
      ...
      "% planowało bezpośrednio po jej ukończeniu rozpocząć studia na uczelni.**",
      sep = ""
    )
  }
}
